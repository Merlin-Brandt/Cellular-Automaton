...
// indices for vertex and fragment shader (independent from OpenGL indices)
...
// rendered life values are stored in this two-dimensional image
static GLubyte *cellsTex;

// render each cell as an rectangle instead of two triangles
#define QUAD_RENDER
...
static char const *shdSrc[2] =
{
    ...
    "void main()"
    "{"
        // choose color from texture according to position of current fragment
...
#if 1
// print prefix and OpenGL error message and exit if an OpenGL error occured
...
static void init_shd()
{
    ...
    for (shdI = 0; shdI < 2; ++shdI)
    {
        ...
        // check compilation status and print error message on failure
    ...
    // link program
    {
        ...
        // check linkage status and print error message on failure
        ...
        // delete shaders object references
        for (shdI = 0; shdI < 2; ++shdI)
        {
...
// updates texture according to the cell's value
