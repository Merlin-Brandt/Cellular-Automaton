...
CellLive *cellLife;
// stores the average values for each cell
// data is structured like cellLife
...
void cells_gen(unsigned seed, int full)
{
	...
	// 7 different generation patterns
	int genMode = rand() % 7;
...
void cells_logic()
{
	// coordinates and index of current cell
    int i, x, y;
    ...
    for (x = 0; x < cellGridW; ++x)
        for (y = 0; y < cellGridH; ++y)
        {
			...
			// if current cell is on an edge of the grid
            if (x == 0 || x == cellGridW-1 || y == 0 || y == cellGridH-1)
            {
                // indicators for which edge the current cell inhabits
                int isXPOff = x + 1 == cellGridW;
				int isXMOff = x - 1 == -1;
				int isYPOff = y + 1 == cellGridH;
				int isYMOff = y - 1 == - 1;

				// the grid is laid out on a torus. as such, if a neighbor coordinate is e.g. (-1, 0),
				// then we treat the cell at (cellGridW-1, 0) as the neighbor
			...
			// current cell does not lie on an edge
            else
            {
                cellNeighborAvg[i]+= cellLife[i + 1];
                cellNeighborAvg[i]+= cellLife[i - 1];
	...
	// calculate life values using average values
    i = 0;
    for (x = 0; x < cellGridW; ++x)
        for (y = 0; y < cellGridH; ++y)
        {
